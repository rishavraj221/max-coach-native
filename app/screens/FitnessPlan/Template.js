...
import { createFitnessPlan, getFullFitnessPlan } from "../../api/fitness";
import useAuth from "../../auth/useAuth";
import authStorage from "../../auth/storage";

const weeks = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];

const workOutArr = ["Warm Up", "Main Movement", "Cool Down"];

const FitnessPlanTemplateScreen = ({ route, navigation }) => {
  ...
  const handleNextBtn = async () => {
    ...
    const api_req_body = {
      ...
      ft_category: getTemplateName(selectTemplate),
      ft_days: selectWeeks,
      ft_item: generateDiItems(),
      ft_dates: diet_plan_dates,
      start_date: diet_plan_dates[0],
      end_date: diet_plan_dates[diet_plan_dates.length - 1],
    };

    await createFitnessPlan(token, api_req_body);

    const result = await getFullFitnessPlan(token, auth.user.id, client.c_id);
    console.log(result);

    // console.log(generateDiItems);
    navigation.navigate(routes.CREATE_FITNESS_PLAN, {
      fitnessPlan: result.data,
      client,
    });
  };
    ...
    <Screen>
      <ScrollView style={{ backgroundColor: "white" }}>
        <View style={styles.header}>
          ...
          <AppText style={styles.pageTitle}>Create Fitness Plan</AppText>
        </View>
...
export default FitnessPlanTemplateScreen;
